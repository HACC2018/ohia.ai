...
def get_model(model_name, n_classes):
    ...
    # load the model
    pretrain_path = re.sub('fine-tune', 'pretrain_plantnet', model_path)
    weight_file = np.sort(glob.glob(f'{pretrain_path}/**.h5'))[-1]
    model = Model(inputs=base_model.input, outputs=outputs)
    model.load_weights(weight_file)

    # remove last layer and freeze remaining
    for layer in model.layers[:-1]:
        layer.trainable = False

    # add final layer
    outputs = layers.Dense(n_classes, activation='softmax')(model.output)
    finetune_model = Model(inputs=model.input, outputs=outputs)
...
if __name__ == '__main__':
    """
    python finetune_with_pretrain.py \
